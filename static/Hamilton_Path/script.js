...
let currentIndex = 0;
let progress = { easy: 0, medium: 0, hard: 0 }; // Track progress as percentage
...
function checkSolution() {
    ...
    if (isValid) {
        ...
        // Update the progress bar based on the difficulty level
        if (currentDifficulty === 'easy' || currentDifficulty === 'medium') {
            progress[currentDifficulty] += 10; // 10% per question for easy and medium
        } else if (currentDifficulty === 'hard') {
            progress[currentDifficulty] += 20; // 20% per question for hard
        }
...
function updateProgress() {
    const totalProgress = progress.easy + progress.medium + progress.hard;
